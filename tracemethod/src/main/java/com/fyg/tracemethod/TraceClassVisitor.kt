...
class TraceClassVisitor(api: Int, cv: ClassVisitor?, var traceConfig: Config, var monitoringTimeThreshold : Long) :
    ClassVisitor(api, cv) {
    ...
    override fun visit(
        ...
        System.out.println("mNeedTracePackageMap :"+traceConfig.mNeedTracePackageMap.forEach {it})
        //是否是配置的需要插桩的类
        name?.let { className ->
